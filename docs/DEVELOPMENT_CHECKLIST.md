...
[x] Этап 9   Графики, отчёты, сессии, сравнение с CFD
[x] Этап 10  Качество, производительность, сборка и релиз
```
...
- [+] `L` → компактная/развёрнутая навигация.
- [+] `R` → показать/скрыть инспектор.
- [+] `F` → открыть график в фокус-режиме.
- [+] `Esc` → выйти из фокус-режима.
...
- [+] Кнопка «Сохранить как PNG» — сохранять в выбранный пользователем путь.
...
- [+] Скользящий RMS отображать на отдельном графике под основным графиком сигнала.

#### 9.2 PDF-отчёт

Создать `infrastructure/writers/pdf_report_writer.py`:

- [+] Функция `export_pdf_report(result: AnalysisResult, output_path: str | Path) -> Path`.
- [ ] Структура отчёта из `docs/05_output_data_specification.md`:
  - [+] Титульная страница: название, файл, дата, версия.
  - [+] Раздел 1: параметры и метаданные анализа.
  - [+] Раздел 2: качество данных (пропуски, выбросы).
  - [+] Раздел 3: временна́я область и статистика.
  - [+] Раздел 4: спектральный анализ и таблица пиков.
  - [+] Раздел 5: физические коэффициенты (Re, St, fs, Vr).
  - [+] Раздел 6: сравнение с CFD (если есть `ValidationResult`).
  - [+] Раздел 7: итоговое заключение, предупреждения и ограничения.
...
- [+] Добавить безопасный сервис `save_project(session, path)`:
...
- [+] Добавить сервис `load_project(path) -> AnalysisSession`.
- [+] В `MainWindow`: «Сохранить проект» / «Открыть проект» в панели действий.
...
- [+] Тест PDF-экспорта — PDF генерируется и содержит > 0 байт.
- [+] Тест сессии — сессия сохраняется и открывается без потери настроек.
- [+] Тест сравнения профилей — два идентичных синтетических профиля дают RMSE < 0.001.

#### Критерии завершения этапа 9

- [+] PDF-отчёт генерируется со светлым фоном и основными инженерными разделами.
...
- [+] Расширить тесты `core/` до ≥ 80% покрытия (достигнуто 98%).
- [+] Добавить тест производительности: полный анализ 60 000 строк завершается < 3 с (1.07 с).
...
- [+] Убедиться: все тесты проходят локально на Windows (372 passed, 1 deselected).
...
- [+] Добавить запись в `CHANGELOG.md`: `## [1.0.0] — 2026-06-11`.
...
- [+] Добавить `hiddenimports` для scipy, pandas, PySide6, reportlab.

- [ ] Собрать командой: `pyinstaller scripts/iva.spec` — требует запуска на целевой машине.
- [ ] Проверить: `dist/IVA/IVA.exe` запускается на машине разработчика.
- [ ] Проверить (если возможно): `dist/IVA/IVA.exe` запускается на чистой Windows 11 без Python.
...
- [+] Версия: `1.0.0` (в скрипте; сборка через `ISCC scripts/installer.iss`).
- [+] Папка установки: `{autopf64}\IVA\`.
...
- [ ] Собрать установщик: `ISCC scripts/installer.iss` — требует Inno Setup 6 на Windows.
...
- [+] Запускает `ISCC scripts/installer.iss` (если Inno Setup найден).
- [+] Копирует `IVA_Setup_{версия}.exe` в `dist/release/`.
- [+] Выводит итог: путь к установщику, размер файла.
- [+] Поддерживает флаги `--skip-tests` и `--check-only`.
...
- [ ] Установщик проверен на установку и удаление (требует Windows + Inno Setup 6).
- [ ] Демонстрационный анализ на демо-файле прошёл успешно (GUI).
- [ ] `dist/release/archive/` содержит предыдущий установщик (на случай отката).

#### 10.8 Финальная документация

- [+] Обновить `README.md` — добавить разделы «Running Quality Checks» и «Building the Windows Installer».
- [+] Обновить `docs/00_project_overview.md` — актуализировать описание первой версии.
- [+] Обновить `docs/22_portfolio_case_study.md` — кейс для портфолио.
- [+] Создать `docs/screenshots/README.md` — инструкция по снятию скриншотов.
- [ ] Сделать скриншоты приложения — требует ручного запуска GUI на Windows.

#### Критерии завершения этапа 10

- [+] Все тесты проходят. Покрытие `core/` ≥ 80% (98%).
- [+] `scripts/build_installer.py` завершается без ошибок (в режиме --check-only и --skip-tests).
- [ ] `IVA_Setup_1.0.0.exe` создан в `dist/release/` — требует Windows + Inno Setup 6.
- [ ] Установленное приложение запускается на Windows 11 — ручная проверка.
- [ ] Демонстрационный анализ работает после установки — ручная проверка.
...
- [+] PDF-отчёт включает основные инженерные разделы.
- [+] Сессия сохраняется и открывается без потери данных.
- [+] Тесты покрывают ≥ 80% кода `core/` (достигнуто 98%).
- [+] Сборка и релиз автоматизированы скриптом `scripts/build_installer.py`.
- [ ] Установщик `IVA_Setup_1.0.0.exe` работает на чистой Windows 11 (требует ручной проверки).
