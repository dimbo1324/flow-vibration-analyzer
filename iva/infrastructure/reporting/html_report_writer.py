...
def export_html_report(result: AnalysisResult, output_path: str | Path) -> Path:
    ...
    try:
        ...
        raise ExportError(
            user_message=f"Cannot write HTML report to '{output_path.name}'.",
...
<head>
  ...
  <title>IVA Report &mdash; {source_name}</title>
  <style>
...
<body>
  <div class="page">
    <header>
      <h1>IVA Analysis Report</h1>
      <p>Source file: <strong>{source_name}</strong></p>
      <p>Session ID: {session_id} &nbsp;&bull;&nbsp; Generated: {generated}</p>
      <p>MD5: {md5}</p>
    </header>
...
{tables_html}
    ...
    <footer>
      <p>Generated by Industrial Vibration Analyzer (IVA).
      This report is for engineering review purposes.</p>
    </footer>
  </div>
</body>
</html>
