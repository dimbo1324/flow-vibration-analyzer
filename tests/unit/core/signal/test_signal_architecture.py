"""Architecture boundary test: core/signal must not import from forbidden layers.

Verifies via AST parsing that no module in iva.core.signal imports from:
    - iva.ui
    - iva.app
    - iva.infrastructure
    - PySide6
...
_SIGNAL_PKG = Path(__file__).resolve().parents[4] / "iva" / "core" / "signal"
